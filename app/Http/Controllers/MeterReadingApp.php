...
class MeterReadingApp extends Controller
{
    ...
    ///calculate if late pay -> interest
        ...
        $CalculatedData=[
            ...
            'AccNo'=> $Acc->id,
            'CusID'=>$customer->id,
            'CusAcc_NO'=>$Acc->CusAcc_No,
            'CusName'=>$customer->Cus_NameInitials,
            'CusAddress'=>$customer->Cus_Address,
            'Cus_No'=>$customer->Cus_id,
            'Cus_phone'=>$customer->Cus_Phone1,
            'date'=>'today',
    ...
    public function ConfirmedData(Request $request) {
        ...
        $CalculatedData = json_decode($request->input('data'), true);
        if ($CalculatedData === null) {
            return redirect()->back()->withErrors('Failed to decode data.');
        }

        //saving data to the database
        $bill = new WaterBills();
        $bill->Bill_Reading1= $CalculatedData['LastReading'];
        $bill->Bill_Reading2= $CalculatedData['NewReading'];
        $bill->Bill_Units= $CalculatedData['NetUnits'];
        $bill->Bill_ChargeUsedUits= $CalculatedData['ChargeForUsedUnits'];
        $bill->Bill_ServiceCharge= $CalculatedData['ServiceCharge'];
        $bill->Bill_BF= $CalculatedData['B/F'];
        $bill->Bill_Tax= $CalculatedData['Tax'];
        $bill->Bill_DisCount= $CalculatedData['Discount'];
        $bill->Bill_Installment= $CalculatedData['Installment'];
        $bill->Bill_Interest= $CalculatedData['Interest'];
        $bill->Bill_Total= $CalculatedData['TotalAmount'];
        $bill->CusAcc_No= $CalculatedData['AccNo'];
        $bill->MTR_id= Session::get('MTR_User');
        $bill->Org_id= Session::get('Org_id');


        $updatereading = Acc_LastReading::where('Acc_id',$CalculatedData['AccNo'])->first();

        if($updatereading && $bill){
            $updatereading->LastReading = $CalculatedData['NewReading'];
            $updatereading->save();
            
            $bill->save();
        
            $Installment = Cus_Installment::where('CusAcc_id',$CalculatedData['AccNo'])->first();

            if($Installment){
                if($Installment->Ins_NoOfRemain > 0){
                    $Installment->Ins_NoOfRemain --;
                    $Installment->save();
                }
            }
            $Acc = Cus_account::findOrFail($CalculatedData['AccNo']);

            $Acc->CusAcc_Balance = -1* $CalculatedData['TotalAmount'];
            $Acc->save();


            return view('Meter.bill', compact('CalculatedData'));
